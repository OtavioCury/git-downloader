...
public class ProjectInfoDAO extends GenericDAO<ProjectInfo> {
	...
	public ProjectInfo findByName(String projeto) {
		String hql = "select a from " +  ProjectInfo.class.getSimpleName() + " a where a.fullName = \'"+projeto+"\' ";
		Query q = em.createQuery(hql);
		return (ProjectInfo) q.getSingleResult();
	}

	public List<ProjectInfo> findByLanguage(String linguagem) {
		String hql = "select a from " +  ProjectInfo.class.getSimpleName() + " a where a.language = \'"+linguagem+"\' ";
		Query q = em.createQuery(hql);
		return q.getResultList();
	}
